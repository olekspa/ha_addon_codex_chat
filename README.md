...
## Custom Integration
- `custom_components/lentus_conversation`: Home Assistant Conversation Agent so Assist can route to Lentus/Codex relay.
...
1. Update `codex_chat/config.yaml` `version` (example: `0.3.9`).
...
4. GitHub Action `.github/workflows/build-addon-images.yml` builds and pushes:
   - `ghcr.io/<owner>/amd64-codex_chat:0.3.9`
   - `ghcr.io/<owner>/aarch64-codex_chat:0.3.9`
   - `ghcr.io/<owner>/armv7-codex_chat:0.3.9`
...
## Install Lentus Assist Agent (custom component)
1. Copy `custom_components/lentus_conversation` into your Home Assistant config:
   - `<ha_config>/custom_components/lentus_conversation`
2. Restart Home Assistant.
3. Go to **Settings -> Devices & Services -> Add Integration**.
4. Add **Lentus Conversation Agent**.
5. Fill relay settings (`relay_url`, `relay_token`) and keep defaults for:
   - `approval_policy=never`
   - `sandbox_mode=danger-full-access`
6. Go to **Settings -> Voice assistants -> Assist** and select the Lentus agent as default.
